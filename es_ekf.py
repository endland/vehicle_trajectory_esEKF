...
# 3D rigid body motion (3D Euclidean transformation) X' = RX + t
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    delta_pk = delta_Xk[:3]
    delta_vk = delta_Xk[3:6]
    delta_phik = delta_Xk[6:]

    p_hat = p_check + delta_pk
    v_hat = v_check + delta_vk
    #q_hat = Quaternion(euler=delta_phik).quat_mult_right(q_check)
    q_hat = Quaternion(axis_angle=delta_phik).quat_mult_right(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_est[k] = p_est[k-1] + delta_t*v_est[k-1] + (delta_t**2/2)*(rot_mat @ imu_f.data[k-1] + g) # Position    
    v_est[k] = v_est[k-1] + delta_t*(rot_mat @ imu_f.data[k-1] + g)                             # Velocity
    q_est[k] = Quaternion(axis_angle=imu_w.data[k-1]*delta_t).quat_mult_right(q_est[k-1])       # Orientation
    ...
    F[3:6, 6:] = -(rot_mat @ skew_symmetric(imu_f.data[k-1].reshape((3,1))))*delta_t  # don't miss delta_t
    ...
    if lidar_i < lidar.t.shape[0] and abs(lidar.t[lidar_i] - imu_f.t[k-1]) < 0.01:
        p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_lidar, p_cov[k], lidar.data[lidar_i], p_est[k], v_est[k], q_est[k])
        lidar_i += 1

    if gnss_i < gnss.t.shape[0] and abs(gnss.t[gnss_i] - imu_f.t[k-1]) < 0.01:
